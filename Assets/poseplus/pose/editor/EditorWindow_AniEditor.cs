...
class EditorWindow_AniEditor : EditorWindow
{
    Vector2 clipfuncpos;
    ...
    public static void Show(FB.PosePlus.Dev_AniEditor editor)
    {
        ...
        var window = EditorWindow.GetWindow<EditorWindow_AniEditor>(false, "AniExEditor");
        //window.title = "才";
...
//                     }
    ...
    public void ShowAniChild(FB.PosePlus.AniClip clip, SubClip a)
    {
        ...
        int frame = (int)(delta * fps) + (int)a.startframe;
        if (frame > a.endframe)
        {
            if (a.loop)
            {
...
//        GUILayout.Space(5);
        ...
        obj =  EditorGUILayout.ObjectField("特 效:",obj,typeof(GameObject),false);
        if (obj != null)
        {
            string path = AssetDatabase.GetAssetPath(obj);
            effectName = path.Replace("Assets/Resource/Runtime/", "").Replace(".prefab", "");
        }
        GUILayout.EndHorizontal();
...
//            {
               Effect e = new Effect();
               e.name = effectName.Replace("Asset/Resource/Runtime/","");
               e.follow = effectAddr;
