...
public class EditorWindow_AniTools : EditorWindow
{
    ...
    {
        var window = EditorWindow.GetWindow<EditorWindow_AniTools>(true, "Window_Tools", true);
    ...
    void OnGUI_ShowLeft()
    {
        GUILayout.BeginVertical();
        {
...

            {
                ...
                if (GUILayout.Button("右移>>", GUILayout.Width(50)))
                {
                    ...
                    if (EditorUtility.DisplayDialog("对比信息",
                        "相同骨骼：" + i.ToString() + "\n不同骨骼：" + j.ToString() + "\n 是否复制到右边？", "Yes", "No"))
                    {
        ...
        GUILayout.EndVertical();
        EditorWindow_AniEditor.Layout_DrawSeparatorV(Color.white);
    }
    ...
    void MoveData(Operation opera)
    {
        ...
        string str = "";
        string plan = "";
        if (opera == Operation.left_box || opera == Operation.left_dot || opera == Operation.left_effct ||
            opera == Operation.left_adudio)
        {
            if (count > 0)
            {
        ...
        else if (opera == Operation.right_box || opera == Operation.right_dot || opera == Operation.right_audio ||
                 opera == Operation.right_effct)
        {
            if (count > 0)
            {
            ...
            if (count >= 0)
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (opera == Operation.left_box || opera == Operation.left_dot || opera == Operation.left_effct ||
                        opera == Operation.left_adudio)
                        clip_b.frames.Add(f);
                    else if (opera == Operation.right_box || opera == Operation.right_dot ||
                             opera == Operation.right_audio || opera == Operation.right_effct)
